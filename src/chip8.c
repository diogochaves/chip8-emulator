...
{
	memset(chip8, 0, sizeof(t_chip8));
	memcpy(
		&chip8->memory.memory,
		chip8_default_character_set,
...
static void	chip8_exec_extended_eight(t_chip8 *chip8, unsigned short opcode)
{
	unsigned short	x = (opcode >> 8) & 0x00f;
	unsigned short	y = (opcode >> 4) & 0x00f;
	unsigned short	final_four_bits = opcode & 0x00f;
	...
	switch(final_four_bits)
	{
		...
		case 0x06:
			chip8->registers.V[0x0f] = chip8->registers.V[x] & 0x01; // ... & 0b00000001
...
static char	chip8_wait_for_key_press(t_chip8 *chip8)
{
	SDL_Event	event;
...
static void	chip8_exec_extended_F(t_chip8 *chip8, unsigned short opcode)
{
	unsigned short	x = (opcode >> 8) & 0x00f;
...
static void	chip8_exec_extended(t_chip8 *chip8, unsigned short opcode)
{
